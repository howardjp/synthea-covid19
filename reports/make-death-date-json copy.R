...
for (i in patient_list) {
    k <- k + 1
    info(logger, paste("Processing patient", k, "of", length(patient_list)))
    ...
    for (j in covid19_tmp[, TINDEX]) {
        ...
        if(is.null(covid.matrix)) covid.matrix <- covid19_subtmp
        else covid.matrix <- rbind(covid.matrix, covid19_subtmp)
...
total_obs <- nrow(covid.matrix) / max_obs

covid.result <- as.matrix(covid.result)

# I am not sure which of the following 3 lines I am most ashamed of
covid.pmatrix <- array(t(covid.matrix), c(ncol(covid.matrix), max_obs, total_obs))
covid.pmatrix <- aperm(covid.pmatrix, c(2, 1, 3))
covid.matrix <- aperm(covid.pmatrix, c(3, 1, 2))
...
write(covid.matrix.json, "synthea/full-x.json")
write(covid.result.json, "synthea/full-y.json")
