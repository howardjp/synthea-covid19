...
covid19.dt <- merge(covid19.obvs.dt, covid19.meds.dt, by = c("PATIENT"), all = TRUE)
covid19.dt <- merge(covid19.dt, covid19.prcs.dt, by = c("PATIENT"), all = TRUE)
...
covid19.dt.names <- c(covid19.meds.dt.names, covid19.prcs.dt.names)
for(i in covid19.dt.names) {
    missingMedsPrcs <- as.vector(is.na(covid19.dt[, ..i]))
    covid19.dt[missingMedsPrcs, ..i := 0]
}
